...
const PhasorGraph = (props) => {
  ...
  function renderSvg() {
    ...
    let margin = {
      top: 33 + zoomState.offset.y,
      bot: 33,
      left: 33 + zoomState.offset.x,
      right: 33,
    };
    ...
    tooltip
      .style("left", 0 + "px")
      .style("top", 0 + "px")
      .style("position", "absolute"); //Tooltip methods declaration
    ...
    let diagYAxisScale = d3
      .scaleLinear()
      .domain([135, 315])
      .range([0, totalWidth * zoomState.zoom]);

    let diagYAxisScale2 = d3
      .scaleLinear()
      .domain([135, 315])
      .range([-15, totalWidth * zoomState.zoom + 15]);

    let diagXAxisScale = d3
      .scaleLinear()
      .domain([225, 45])
      .range([
        !expanded ? -1 - zoomState.zoom : 0,
        totalWidth * zoomState.zoom + (!expanded ? -1 - zoomState.zoom : 0),
      ]);

    let diagXAxisScale2 = d3
      .scaleLinear()
      .domain([225, 45])
      .range([-15, totalWidth * zoomState.zoom + 15]);

    let max = d3.max(props.vectorData, (d) => {
      return d.amp;
    });
    if (!max) {
      max = 20;
    ...
    const xAxisGroup = svg
      ...
      .attr(
        "transform",
        `translate(${
          margin.left + (totalWidth / 2) * ((zoomState.zoom - 1) * -1)
        },${totalHeight / 2 + margin.top})`
      )
      .attr("id", "x-axis")
      .call(xAxis);
    ...
    const yAxisGroup = svg
      ...
      .attr(
        ...
        "translate(" +
          ...
          "," +
          (margin.top + (totalHeight / 2) * ((zoomState.zoom - 1) * -1)) +
          ")"
      )
      .call(yAxis);
    ...
    //Append the amp-Axis
    const ampAxisGroup = svg
      .append("g")
      .attr("id", "amp-axis")
      .attr(
        "transform",
        `translate(${margin.left + totalWidth / 2} ${
          totalHeight / 2 + margin.top
        }) rotate(-45)`
      )
      .call(ampAxis);
    ...
    const diagpointX1 =
      margin.left +
      totalWidth / 2 -
      ampAxisScale(max * (3 / 4)) +
      4.5 * zoomState.zoom;
    const diagpointX2 =
      margin.left +
      totalWidth / 2 -
      ampAxisScale(max * (3 / 4)) +
      (expanded ? 3 : 7) * zoomState.zoom;
    const diagpointY1 =
      margin.top +
      totalHeight / 2 -
      ampAxisScale(max * (3 / 4)) +
    ...
    const diagpointY2 =
      ...
      margin.top +
      Math.sqrt(
        Math.pow(ampAxisScale(max), 2) -
          Math.pow(ampAxisScale(max * (3 / 4)), 2)
      ) +
      4.5 * zoomState.zoom;
    ...
    const yDiagAxisGroup = svg
      .append("g")
      .attr(
        "transform",
        "translate(" + diagpointX1 + "," + diagpointY1 + ") rotate(45)"
      )
      .attr("id", "xDiag-axis")
      .call(yDiagAxis);
    ...
    const yDiagAxisGroup2 = svg
      .append("g")
      .attr(
        "transform",
        "translate(" + diagpointX1 + "," + (diagpointY1 - 10) + ") rotate(45)"
      )
      .attr("id", "xDiag-axis")
      .call(yDiagAxis2);
    ...
    const xDiagAxisGroup = svg
      .append("g")
      .attr(
        "transform",
        "translate(" + diagpointX2 + "," + diagpointY2 + ") rotate(-45)"
      )
      .attr("id", "xDiag-axis")
      .call(xDiagAxis);
    ...
    const xDiagAxisGroup2 = svg
      .append("g")
      .attr(
        "transform",
        "translate(" + diagpointX2 + "," + (diagpointY2 - 10) + ") rotate(-45)"
      )
      .attr("id", "xDiag-axis")
      .call(xDiagAxis2);
    ...
    <div className="phasor_graph_container">
      ...
      <div className="zoom_controls_container">
        <div className="zoom_controls_group_one">
          <Button
            className="zoom_plus zoom_control"
            onClick={() => {
              setzoomState({ ...zoomState, zoom: zoomState.zoom + 0.5 });
            }}
          >
            +
          </Button>
          <Button
            className="zoom_minus zoom_control"
            onClick={() => {
              setzoomState({ ...zoomState, zoom: zoomState.zoom - 0.5 });
            }}
          >
        ...
        <div className="zoom_controls_group_two">
          <Button
            className="zoom_leftarr zoom_control"
            onClick={() => {
              setzoomState({
                ...zoomState,
                offset: { ...zoomState.offset, x: zoomState.offset.x + 15 },
              });
            }}
          >
            {"<"}
          ...
          <div className="zoom_controls_up_down">
            <Button
              className="zoom_uparr zoom_control"
              onClick={() => {
                setzoomState({
                  ...zoomState,
                  offset: { ...zoomState.offset, y: zoomState.offset.y + 15 },
                });
              }}
            >
              {"<"}
            ...
            <Button
              className="zoom_downarr zoom_control"
              onClick={() => {
                setzoomState({
                  ...zoomState,
                  offset: { ...zoomState.offset, y: zoomState.offset.y - 15 },
          ...
          <Button
            className="zoom_rightarr zoom_control"
            onClick={() => {
              setzoomState({
                ...zoomState,
                offset: { ...zoomState.offset, x: zoomState.offset.x - 15 },
